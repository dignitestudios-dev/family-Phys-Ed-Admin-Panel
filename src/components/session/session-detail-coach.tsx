...
function SessionDetail({
  ...
  data: PublicSessionDetail | PrivateSessionDetail;
}) {
        ...
        <div className="bg-secondary flex justify-between items-center p-4 rounded-2xl text-white">
          <div className="">
            <h1 className="text-2xl">
              {data?.title}{" "}
              <span className="bg-primary text-black text-sm p-1 px-2 rounded-full">
                {data?.session_type}
              </span>
            </h1>
          ...
          <h1
            className={getSessionStatusBadgeClassName(
              data?.status
            ) + " rounded-full px-3 py-2 text-center text-sm font-semibold"}
          >
            {formatSessionStatus(data?.status)}
          </h1>
        </div>
              ...
              <div className="text-xs border-b pb-2">
                <div className="flex gap-4 py-2">
                  <h1 className="flex gap-2 items-center">
                    {" "}
                    <CalendarDays className="text-primary" size={15} />
                    {data?.date}
                  </h1>
                  <h1 className="flex gap-2 items-center">
                    {" "}
                    <Clock2 className="text-primary" size={15} />
                    {data?.duration}
                  </h1>
                </div>
                ...
                <h1 className="flex items-center gap-2">
                  {" "}
                  <MapPin className="text-primary" size={20} /> {data?.location}
                </h1>
              </div>
                ...
                <div className="bg-[#2c2c2e] flex justify-between items-center p-2 rounded-2xl px-4">
                  <div className="flex gap-2 items-center">
                    <Image
                      ...
                      src={`${process.env.NEXT_PUBLIC_MEDIA_URL}${data?.requested_user.avatar}`}
                      alt="img"
                      width={50}
                      height={50}
                    />
                    ...
                    <div>
                      {" "}
                      <h1 className="text-sm">{data?.requested_user.name}</h1>
