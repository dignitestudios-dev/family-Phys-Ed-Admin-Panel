...
const Requests = () => {
    ...
    const { loading, users, totalPages, getAllRequests } = getHooks.useGetAllProfileReq();
    ...
    const currentList = selectedTab === "0" ? users?.profile_requests : users?.merchandise_requests
                ...
                <div className="px-4">
                    ...
                    <div className="flex bg-[#1C1C1E] items-center w-fit p-1 rounded-sm gap-5">
                        {tabs.map((tab, index) => (
                            <p
                                key={index}
                                className={`cursor-pointer text-sm text-center px-10 w-[250px] py-1 ${selectedTab === String(index)
                    ...
                    {item?.is_deactivate ? "Inactive" : "Active"}
                        ...
                        users?.profile_requests.data?.map((item, index) => (
                                <tr key={index} onClick={() => router.push(`/user-management/${item.uid}?role=coach`)} className="border-b cursor-pointer border-[#3a3a3c]">
                                    ...
                                    <td
                                        className={`px-4 py-6 ${item?.is_approved ? "text-[#EE0004]" : "text-[#85D500]"
                                            }`}
                                    >
                                        {item?.is_approved }
                                    </td>
